...
def read_data(args, tokenizer):
    ...
    for i, row in tqdm(test_df.iterrows(), desc=f'Preprocessing test data', total=len(test_df)):
        sentence_a, sentence_b = row[0].strip(), row[1].strip()
        build_bert_inputs(inputs, sentence_a, sentence_b, tokenizer)
...
def build_bert_inputs(inputs, sentence_a, sentence_b, tokenizer):
